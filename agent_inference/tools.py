...
class GTR:
    ...
    def rerank(self, query, docs):
        ...
        if len(doc_embs) == 0 or len(query_emb) == 0:
            return docs
...
class SearchTool(Tool):
    ...
    def search(self, query, k=3):
        ...
        citation_pattern_docid = re.compile(r'\[\d+#\d+(?:\s*[-,]\s*\d+#\d+)*\](?:\[\d+#\d+\])*')
        query = citation_pattern_docid.sub("", query)
        print("querying tool with:",query)
        ...
        if self.ranker_type == "GTR":
            ...
            docs_text = [{k: v for k, v in eval(d).items() if k not in ['score','title']} for d in docs_text]
